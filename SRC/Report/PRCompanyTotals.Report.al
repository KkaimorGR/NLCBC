...
report 52193527 "PR Company Totals"
{
    DefaultLayout = RDLC;
    ...
    dataset
    {
        dataitem(prTransactionCodes; "PR Transaction Codes")
        {
            ...
            column(MonthCompanyTotals; MonthCompanyTotals)
            { }
            column(PensionArreas; PensionArreas)
            {
            ...
            begin
                // MonthCompanyTotals := 0;
                //error('%1',totded);
                IF prTransactionCodes."Transaction Code" = 'HOUSING LEVY' THEN CurrReport.SKIP;
                ...
                Prperid.Reset();
                // PeriodTrans.SETRANGE(PeriodTrans."Payroll Code","prPayroll Type"."Payroll Code");
                Prperid.SetFilter(Prperid."Transaction Code", prTransactionCodes."Transaction Code");
                Prperid.SetRange(Prperid."Payroll Period", SelectedPeriod);
                Prperid.SetRange("Transaction Type", Prperid."Transaction Type"::Deduction);
                Prperid.SetFilter(Prperid.Amount, '>%1', 0);
                if Prperid.Find('-') then
                    repeat
                        HDecimal := HDecimal + Prperid.Amount;
                    until Prperid.Next() = 0;
                ...
                // if PeriodTrans.Find('-') then
                //     repeat
                //         HDecimal := HDecimal + PeriodTrans.Amount;
                //     until PeriodTrans.Next() = 0;
                if decAmount = 0 then
                    CurrReport.Skip();
                // MonthCompanyTotals := MonthCompanyTotals + HDecimal;

                // MonthCompanyTotals := NSSFAmount + NPAY + PensioncompanyInfoBal + NITAAmount + HSELEVY + HSELEVYARREAR + HDecimal + TOTDED;
            end;
            ...
            begin
                ...
                if PeriodTrans.Find('-') then
                    repeat
                        NHaMT2 := NHaMT2 + PeriodTrans.Amount;
                    until PeriodTrans.Next() = 0;
                //----------------------------------------------------------------------------------------
               ...
               */
                ...
                */
                ...
                if PeriodTrans.Find('-') then
                    repeat
                        PensionArreas := PensionArreas + PeriodTrans.Amount;
                    until PeriodTrans.Next() = 0;
                PRPeriodTrans.Reset();
                ...
                */
                ...
                TOTDED := TOTDED + PAYEAmount + NSSFAmount + NHIFAmount + HSELEVY + NHaMT2;
                // MonthCompanyTotals := MonthCompanyTotals + NSSFAmount + NPAY + PensioncompanyInfoBal + NITAAmount + HSELEVYARREAR + HDecimal + TOTDED + PensionArreas;
                MonthCompanyTotals := MonthCompanyTotals + GPAY + PensioncompanyInfoBal + PensionArreas + NSSFAmount + NITAAmount + HSELEVYARREAR + HSELEVY;

                // MonthCompanyTotals := NSSFAmount + NPAY + PensioncompanyInfoBal + NITAAmount + HSELEVY + HSELEVYARREAR + HDecimal + TOTDED;

                // MonthCompanyTotals := MonthCompanyTotals + NSSFAmount + PensioncompanyInfoBal + NPAY + NITAAmount + HSELEVY + HSELEVYARREAR + HDecimal + TOTDED;
                // MonthCompanyTotals := MonthCompanyTotals + NSSFAmount + PensioncompanyInfoBal + NPAY + NITAAmount + HSELEVY + HSELEVYARREAR + HDecimal + TOTDED;

                // MESSAGE('%1%2%3%4%5%6', NITAAmount, NSSFAmount, PensioncompanyInfoBal, HSELEVY, TOTDED);
    ...
    var
        myInt: Integer;
    begin
        // Message('dec' + FORMAT(decAmount));
        // Message('tot' + Format(TOTDED));
        // Message(Format(NSSFAmount + NPAY + PensioncompanyInfoBal + NITAAmount + HSELEVY + HSELEVYARREAR + HDecimal + TOTDED));
        // MonthCompanyTotals := NSSFAmount + NPAY + PensioncompanyInfoBal + NITAAmount + HSELEVY + HSELEVYARREAR + HDecimal + TOTDED;
        // Message(Format(NSSFAmount + NPAY + PensioncompanyInfoBal + NITAAmount + HSELEVY + HSELEVYARREAR + HDecimal + TOTDED));
    end;

report 52193425 "QRCODE And Tags"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layouts/QRCODE And Tags.rdlc';
    ApplicationArea = All;
    Caption = 'QRCODE And Tags';
    dataset
    {
        dataitem("Fixed Asset"; "Fixed Asset")
        {
            ...
            begin
                ...
                if StoreHeadears.Find('-') then begin
                    ...
                    HrEmployees.Reset();
                    HrEmployees.SetRange("No.", "Fixed Asset"."Staff No");
                    if HrEmployees.FindFirst() then
                        RequID := HrEmployees."Job Title";
                end;
                FixedAseet.Reset();
                FixedAseet.SetRange("No.", "No.");
                //FixedAseet.SetFilter(Status, '=%1', FixedAseet.Status::Approved);
                if FixedAseet.Find('-') then begin
                    UserSetup.Reset();
    ...
    var
        ...
        FixedAseetRec: Record "Fixed Asset";
        StoreHeaders: Record "Fixed Asset";
        i: Integer;

    begin

        FixedAseet.Reset();
        FixedAseet.setrange(FixedAseet."No.", PvNo);
        //FixedAseet.setrange(FixedAseet.Status, FixedAseet.Status::Approved);
        if FixedAseet.find('-') then begin
            PreparedByUser := UserId;
            StoreHeaders.Reset();
            StoreHeaders.SetRange("No.", FixedAseet."No.");
            if StoreHeaders.FindFirst() then
                // if UserSetup.Get(FixedAseet."Sender ID") then
                if HrEmp.Get(StoreHeaders."Staff No") then EmpName := '; Staff Info: No: ' + HrEmp."No." + ' : Names: ' + HrEmp."First Name" + ' ' + HrEmp."Middle Name" + ' ' + HrEmp."Last Name";
            B64 := QRCodeUnit.GenerateApprovalQRCode(5600, PvNo, EmpName, CurrentDateTime, HrEmp."OTP Code", HrEmp."County Name");
            ...
            FixedAseetRec.Reset;
            // FixedAseetRec.SetRange("Table ID", 52193901);
            FixedAseetRec.SetRange(FixedAseetRec."No.", PvNo);
            // FixedAseetRec.SetRange(Status, FixedAseetRec.Status::Approved);
            if FixedAseetRec.FindSet() then begin
                i := 0;
                repeat
                    i := i + 1;
                    if FixedAseetRec."No." <> '' then begin
                        PreparedByUser := UserId;
                        StoreHeaders.Reset();
                        StoreHeaders.SetRange("No.", FixedAseet."No.");
                        if StoreHeaders.FindFirst() then
                            if HrEmp.Get(StoreHeaders."Staff No") then EmpName := '; Staff: No: ' + HrEmp."No." + ' : Names: ' + HrEmp."First Name" + ' ' + HrEmp."Middle Name" + ' ' + HrEmp."Last Name";
                        B64 := QRCodeUnit.GenerateApprovalQRCode(5600, PvNo, EmpName, CurrentDateTime, HrEmp."OTP Code", HrEmp."County Name");
                        // Bytes := Convert.FromBase64String(B64);
                        // MemoryStream := MemoryStream.MemoryStream(Bytes);
                        //
                    ...
                    if FixedAseetRec."No." <> '' then begin
                        ...
                        if UserSetup.Get(userid) then
                            if HrEmp.Get(UserSetup."Employee No.") then EmpName := '; Staff: No: ' + HrEmp."No." + ' : Names: ' + HrEmp."First Name" + ' ' + HrEmp."Middle Name" + ' ' + HrEmp."Last Name";
                        B64 := QRCodeUnit.GenerateApprovalQRCode(5600, PvNo, EmpName, CurrentDateTime, HrEmp."OTP Code", HrEmp."Cell Phone Number");
                        // Bytes := Convert.FromBase64String(B64);
                        // MemoryStream := MemoryStream.MemoryStream(Bytes);
                        B64.GetSubText(Txt, 1, B64.Length);

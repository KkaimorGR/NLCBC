...
report 52193568 "PR Individual Payslip Portal"
{
    DefaultLayout = RDLC;
    ...
    dataset
    {
        dataitem("HR Employees"; "HR Employees")
        {
            ...
            column(JobTitle_HREmployees; JobTitel) { }
            ...
            dataitem("PR Salary Card"; "PR Salary Card")
            {
                ...
                begin
                    ...
                    RecordNo := RecordNo + 1;
                    ColumnNo := ColumnNo + 1;
                    //Get the staff details (header)  JobHistory.Reset();
                    HREmployeePR.Reset();
                    HREmployeePR.SetRange(HREmployeePR."No.", "Employee Code");
                    if HREmployeePR.FindFirst() then begin
                        JobHistory.Reset();
                        JobHistory.SetFilter(JobHistory."Period Filter", Format(PeriodFilter));
                        JobHistory.SetRange("No.", HREmployeePR."No.");
                        if JobHistory.FindFirst() then begin
                            JobHistory.CalcFields(JobHistory."Job Title");
                            JobID := JobHistory."Job Title";
                        end
                        else
            ...
            begin
                ...
                */
                // JobHistory.Reset();
                // JobHistory.SetFilter(JobHistory."Period Filter", Format(PeriodFilter));
                // JobHistory.SetRange("No.", "HR Employees"."No.");
                // if JobHistory.FindFirst() then
                //     JobID := JobHistory."Job Title"
                // else
                //     JobID := "HR Employees"."Job Title";
                ...
                if HREmployeePR.Find('-') then
                    repeat
                        ...
                        DimensionValue.Reset();
                        // DimensionValue.SetRange(DimensionValue."Global Dimension No.", 1);
                        DimensionValue.SetRange(DimensionValue.Code, County);
                        if DimensionValue.Find('-') then
    ...
    begin
        ...
        PRPayrollPeriods.Reset();
        PRPayrollPeriods.SetRange(PRPayrollPeriods.Closed, false);
        PRPayrollPeriods.SetFilter(PRPayrollPeriods."Date Opened", Format(PeriodFilter));
        ...
        if PeriodName = '' then begin
            ...
            if PRPayrollPeriods.FindFirst() then
                PeriodName := PRPayrollPeriods."Period Name";
            // PeriodName := Format(SelectedPeriod);
        end;

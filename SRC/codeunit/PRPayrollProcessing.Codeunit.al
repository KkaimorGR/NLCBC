...
codeunit 52193424 "PR Payroll Processing"
{
    var
    ...
    var
        ...
        SpecialTransType: Option Ignore,"Defined Contribution","Home Ownership Savings Plan","Life Insurance","Owner Occupier Interest","Prescribed Benefit","Salary Arrears","Staff Loan","Value of Quarters",Morgage,"Post Retirement";
    ...
    begin
        ...
        begin
            ...
            postRetiremax := VitalSetup."Post Retirement Max";
    ...
    procedure Pensionarreas(SelectedPeriod: date)
    var
        myInt: Integer;
        strTableName: Text[50];
        curTransAmount: Decimal;
        ...
        prTransactionCodes: Record "PR Transaction Codes";
        strExtractedFrml: Text[250];
        SpecialTransType: Option Ignore,"Defined Contribution","Home Ownership Savings Plan","Life Insurance","Owner Occupier Interest","Prescribed Benefit","Salary Arrears","Staff Loan","Value of Quarters",Mortgage,Pension,"Mortgage Relief","Post Retirement";
        TransactionType: Option Income,Deduction;
        curPensionCompany: Decimal;
        curTaxOnExcessPension: Decimal;
        prUnusedRelief: Record "prUnused Relief";
    ...
    begin
        ...
        // prEmployeeTransactions.SetRange("Employee Code", 'NLCPEN');
        ...
        begin
           ...
           curNITA_Base_Amount:=0;
            ...
            //Update Employer deductions
            if blnPaysNhif then
                TestHLAmT := Round(fnGetEmployeeNHIF(curGrossPay));
            if PaysNITA then
                CurrNITA := fnGetEmployeeNITA(curNITA_Base_Amount);
            curTransAmount := CurrNITA;
            ...
            curGrossTaxable := curGrossPay + curBenefits + curValueOfQuarters - CurrHSE - TestHLAmT;
            ...
            if curPensionStaff > 0 then begin
                if curPensionStaff > curMaxPensionContrib then
                    //greencom
                    curTransAmount := Round(curMaxPensionContrib, 0.01, '>')
            ...
            if blnPaysPaye and blnGetsPAYERelief then begin
                ...
                //Mortage Relief
                PenReieftAmt := 0;
                PenReieftAmt := fnGetSpecialTransAmount(strEmpCode, intMonth, intYear,
                SpecialTransType::"Post Retirement", false);
                if PenReieftAmt > 0 then begin
                    curTransAmount := PenReieftAmt;
                    strTransDescription := 'Post Retirement Relief';
                    TGroup := 'TAX CALCULATIONS';
                    TGroupOrder := 6;
                    TSubGroupOrder := 9;
                    fnUpdatePeriodTrans(strEmpCode, 'POST-RETIRE-RL', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription,
                    curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept,
                    '', JournalPostAs::Credit, JournalPostingType::" ", '', CoopParameters::none);
                end;
                //End Added
            ...
            */
            ...
            if blnPaysNhif then begin
                curNHIF := Round(fnGetEmployeeNHIF(curNhif_Base_Amount));
                curTransAmount := curNHIF;
                ...
                TGroupOrder := 7;
                TSubGroupOrder := 2;
                fnUpdatePeriodTrans(strEmpCode, 'SHA', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription,
                 curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept,
            ...
            //     if curNHIF > 0 then begin
            //         //MESSAGE(strEmpCode);
            //         curNHIFReliefAmount := Round((PRPivotal."Insurance Relief" / 100) * (curNHIF));
            //         curTransAmount := curNHIFReliefAmount;
            //         strTransDescription := 'S.H.A Relief';
            //         TGroup := 'TAX CALCULATIONS';
            //         TGroupOrder := 6;
            //         TSubGroupOrder := 10;
            //         fnUpdatePeriodTrans(strEmpCode, 'S.H.A Relief', TGroup, TGroupOrder, TSubGroupOrder, strTransDescription,
            //         curTransAmount, 0, intMonth, intYear, '', '', SelectedPeriod, Dept,
            //        '', JournalPostAs::Credit, JournalPostingType::" ", '', CoopParameters::none);
            //     end;
            // end;
    ...
    procedure fnGetSpecialTransAmount(strEmpCode: Code[20]; intMonth: Integer; intYear: Integer; intSpecTransID: Option Ignore,"Defined Contribution","Home Ownership Savings Plan","Life Insurance","Owner Occupier Interest","Prescribed Benefit","Salary Arrears","Staff Loan","Value of Quarters",Mortgage,Pension,"Mortgage Relief","Post Retirement"; blnCompDedc: Boolean) SpecialTransAmount: Decimal
    var
        prEmployeeTransactions: Record "PR Employee Transactions";
        prTransactionCodes: Record "PR Transaction Codes";
        strExtractedFrml: Text[250];
            ...
            repeat
                ...
                if prEmployeeTransactions.Find('-') then
                    ...
                    case intSpecTransID of
                        ...
                        intSpecTransID::"Post Retirement":
                            begin
                                ...
                                if SpecialTransAmount > postRetiremax then
                                    SpecialTransAmount := postRetiremax;
                            end;
    ...
    var
        ...
        PyrollSet: Record "prPayroll Setup";
    begin
        PyrollSet.Get();
        NHIF := (PyrollSet."S.H.A" * curBaseAmount) * 0.01;
        // prNHIF.Reset();

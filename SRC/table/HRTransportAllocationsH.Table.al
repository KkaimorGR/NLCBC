...
table 52193543 "HR Transport Allocations H"
{
    ...
    fields
    {
        ...
        {
            TableRelation = "HR Employees"."No." where("Job Title" = filter('Drive*'), "On Leave" = filter(false), "Vehicle Allocated" = const(false), Status = filter(Active));
            Caption = 'Assigned Driver';
            trigger OnValidate()
            var
                TransportAlLines: Record "HR Transport Allocations H";
                TransportHeeader: Record "HR Transport Requisition";
                Alocated: Integer;
            begin
                HREmp.Reset();
                ...
                TransportHeeader.Reset();
                TransportHeeader.SetRange(TransportHeeader.Status, Status::Released);
                if TransportHeeader.FindFirst() then begin
                    repeat
                        TransportAlLines.Reset();
                        TransportAlLines.SetRange(TransportAlLines."Assigned Driver", Rec."Assigned Driver");
                        TransportAlLines.SetRange(TransportAlLines."Transport Allocation No", TransportHeeader."Transport Request No");
                        if TransportAlLines.Find('-') then begin
                            Alocated := TransportAlLines.Count;
                            "No of trips Attained" := Alocated;
                        end;
                    until TransportHeeader.Next() = 0;
                end;
            end;
        }
        ...
        field(82; "Fuel Litres Allocated"; Decimal)
        {
            DataClassification = ToBeClassified;
        }
        field(83; "Total Fuel Cost"; Decimal)
        {
            DataClassification = ToBeClassified;
        }
        field(88; "No of trips Attained"; Integer)
        {
            DataClassification = ToBeClassified;
            // CalcFormula = count("HR Transport Requisition" where("Transport Request No" = field("Transport Allocation No"), Status = const(Approved)));

...
table 52193601 "Staff Advance Headers"
{
    ...
    fields
    {
        ...
        field(35; Status; Option)
        {
            ...
            Caption = 'Status';
            OptionCaption = 'Open,"1st Approval","2nd Approval","Cheque Printing",Posted,Cancelled,Checking,VoteBook,"Pending Approval",Approved,Rejected';
            Editable = false;
        ...
        field(482; "Employee No"; Code[20])
        {
            ...
            var
                ...
                TXT0001: Label 'You have an outstanding Advance %1. Please clear the outstanding balance to proceed.';
            begin
                HREmp.Reset();
        ...
        field(483; "Employee Names"; Text[100])
        {
            ...
            begin
                ...
                ClsedDate := 0D;
                Prperiods.Reset();
                Prperiods.SetRange(Closed, true);
                if Prperiods.findlast then begin
                    ClsedDate := Prperiods."Date Opened";
                ...
                if ImprestHeader.FindFirst() then begin
                    ...
                    if StaffAdLines.FindLast() then begin
                        ...
                        StaffAdLines.No := ImprestHeader."No.";
                        StaffAdLines."Employee No" := Rec."Employee No";
                        // StaffAdLines.Insert(true);
                        // exit;
                        StaffAdLines."Date Taken" := ImprestHeader.Date;
                        // ImprestHeader.TestField("Responsibility Center");
                        StaffAdLines."Employee No" := rec."Employee No";
                        StaffAdLines."Employee Name" := rec."Employee Names";
                        ...
                        PRPeriod.SetRange(PRPeriod."Payroll Period", ClsedDate);
                        if PRPeriod.Find('-') then
                            StaffAdLines."A Third Basic Pay" := Round(hrsetup."Basic Muiltipier" * PRPeriod.Amount, 0.01, '>');
                        ...
                        if PRPeriod.Find('-') then begin
                            ...
                            StaffAdLines."Emeployee Gross Pay" := "Gross Pay";
                        end;
                        ...
                        PRPeriod.SetRange(PRPeriod."Payroll Period", ClsedDate);
                        if PRPeriod.Find('-') then
                            StaffAdLines."Net Pay" := Round(PRPeriod.Amount, 0.01, '>');

                        //if Amount > "A Third Basic Pay"  then
                        //  Error('The Salary advance Amount Requested is higher than 3 times your Gross Pay');
                        StaffAdLines."A Third Rule" := 1 / 3 * "Gross Pay";
                        StaffAdLines."Max.Amount Per 1/3 Rule" := StaffAdLines."Net Pay" - StaffAdLines."A Third Rule";
                        if "No.of Months to Repay" <> 0 then
                            "Monthly Repayment Amount" := StaffAdLines.Amount / "No.of Months to Repay";
                        // if "Monthly Repayment Amount" <> 0 then
                        //     if StaffAdLines."Net Pay" - "Monthly Repayment Amount" < (1 / 3 * "Gross Pay") then
        ...
        field(487; "Reasons for Advance Amount"; Text[250])
        {
            ...
            begin
                ...
                if ImprestHeader.FindFirst() then begin
                    StaffAdLines.Reset();
                    StaffAdLines.SetRange(No, rec."No.");
                    if StaffAdLines.FindFirst() then begin
                        StaffAdLines."Date Taken" := ImprestHeader.Date;
                        // ImprestHeader.TestField("Responsibility Center");
                        StaffAdLines."Employee No" := Rec."Employee No";
                        StaffAdLines."Employee Name" := rec."Employee Names";
                        ...
                        PRPeriod.SetRange(PRPeriod."Payroll Period", ClsedDate);
                        if PRPeriod.Find('-') then begin
                            StaffAdLines."Emeployee Gross Pay" := PRPeriod.Amount;
                            StaffAdLines."A Third Basic Pay" := Round(hrsetup."Basic Muiltipier" * PRPeriod.Amount, 0.01, '>');
                        end;
                        StaffAdLines.Modify()
                    end else begin

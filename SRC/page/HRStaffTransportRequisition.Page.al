...
page 52194227 "HR Staff Transport Requisition"
{
    PageType = Card;
    ...
    layout
    {
        area(Content)
        {
            group(General)
            {
                ...
                field("Departure Area"; "Departure Area")
                {
                    ApplicationArea = all;
                }
        ...
        area(FactBoxes)
        {
            part("Attached Documents"; "Document Attachment Factbox")
            {
                ApplicationArea = All;
                Caption = 'Attachments';
                SubPageLink = "Table ID" = CONST(52193487),
                              "No." = FIELD("Transport Request No");
            }
        }
    }
            ...
            group("Workflow Process")
            {
                        ...
                        if HRTransportReq.FindFirst() then begin
                            ...
                            if Passngerline.FindFirst() then begin
                                ...
                                if TransportAloo.FindFirst() then begin
                                    if TransportAloo."Assigned Driver" = '' then
                                        Error('you must assign the driver for the trip');
                ...
                action("Re-open Document")
                {
                    ...
                    var
                        myInt: Integer;
                    begin
                        if Rec.Status = Rec.Status::Released then error('You cannot reopen a released vehicle');
                ...
                action(Print)
                {
                    ...
                    begin
                        ...
                        if HRLeaveApp.Find('-') then
                            Report.Run(52193451, true, true, HRLeaveApp);
                    end;
                }
                action("BC Attachments")
                {
                    ApplicationArea = All;
                    Caption = 'BC Attachments';
                    Image = Attach;
                    Promoted = true;
                    PromotedCategory = Category5;
                ...
                action("Modify Allocated Days")
                {
                    ...
                    PromotedOnly = true;
                    Image = Calculate;
                    Caption = 'Modify Allocated Days';
                    trigger OnAction()
                    var
                        myInt: Integer;
                    begin
                        if Confirm('Do you want to Make the changes on the allocated days?') = true then begin
                            "Add Days ?" := true;
                            EditValues := true;
                            Rec.Modify();
                            Message('Proceed to make the necessary changes');
                        end;

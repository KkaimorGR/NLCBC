...
page 52193696 "HR Setup"
{
    ...
    layout
    {
        area(Content)
        {
            group(General)
            {
                ...
                field("OTP Timeout"; "OTP Timeout")
                {
                    ApplicationArea = all;

                }
                field("Allow send mails"; "Allow send mails")
                {
                    ApplicationArea = all;
                }
            }
            ...
            group(Appraisal)
            {
                ...
                field("Basic Muiltipier"; "Basic Muiltipier")
                {
                    ApplicationArea = all;
                }
                field(SharePointUrl; SharePointUrl)
                {
                    ApplicationArea = all;
                }
                field(deleteSharepointUrl; deleteSharepointUrl)
                {
                    ApplicationArea = all;
                }
            }
    ...
    begin
        ...
        if not UserSet.FindFirst() then
            Error('You cannot modify or amend HR setups');
    end;
}

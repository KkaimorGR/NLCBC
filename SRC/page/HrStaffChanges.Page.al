...
page 52193463 "HR Staff Changes"
{
    // DeleteAllowed = false;
    PageType = Card;
    ...
    layout
    {
        area(Content)
        {
            group("General Details")
            {
                ...
                field("Global Dimension 1 Code"; Rec.County)
                {
                    ApplicationArea = all;
                    Caption = 'Home County';
                }
                ...
                field("Can Issue"; "Can Issue")
                {
                    ApplicationArea = Basic;
                    ToolTip = 'Specifies the value of the Can Issue? field.';
                }
                field(County; Rec."Global Dimension 1 Code")
                {
                    ApplicationArea = Basic;
                    Caption = 'Workstation';
                ...
                {
                    ApplicationArea = all;
                    Caption = 'Highest Level Qalification';
                }
            ...
            group("Job Details")
            {
                ...
                field("Position Title"; Rec."Job Title5")
                {
                    ApplicationArea = Basic;
                    Caption = 'Acting Title';
                    Editable = false;
                    ToolTip = 'Specifies the value of the Position Acting field.';

                }
                field("Department Acting"; Rec."Department acting")
                {
                    ApplicationArea = Basic;
    ...
    actions
    {
        area(Navigation)
        {
            ...
            group("&Employee")
            {
                ...
                action("Histry changes")
                {
                    ApplicationArea = all;
                    Image = History;
                    Caption = 'History Changes';
                    Promoted = true;
                    PromotedCategory = Category5;
                    PromotedIsBig = true;
                    RunObject = Page "Hr Change log values";
                    RunPageLink = "Employee No" = field("Employee No"), "Old Value" = filter(<> '');
                }
            ...
            group(ActionGroup19)
            {
                ...
                action("Tuskie Ngoma")
                {
                    ...
                    trigger OnAction()
                    var
                        Portal: Codeunit PortalWebService;
                            ...
                            if HREmp.Find('-') then begin
                                repeat
                                    ...
                                    if LeaveJournalLine.FindLast() then begin
                                        ...
                                        LeaveJournalLine."Leave Calendar Code" := '2026/2027';
                ...
                action(ClearValue)
                {
                    ...
                    var
                        myInt: Integer;
                        valueEntr: Record "Value Entry";
                    begin
                        valueEntr.Reset();
                        if valueEntr.FindSet() then
                            valueEntr.DeleteAll();
                        Message('Done');
                ...
                action(ExportDoc)
                {
                    ...
                    var
                        ...
                        portalchanges: Codeunit PortalWebService;
                    begin
                        //portalchanges.deleteAttachDocs('PREQ-0191',94);
                        // eRecruitment.FnAcademicQualifications('CND00032', 2, 'Nairobi', 'Albania', 'BA_206', '', 0, '2000', 20170101D, 20190101D, '', 0, 0, '', '');
                        //52193435
